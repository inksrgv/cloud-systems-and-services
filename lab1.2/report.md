...
Проверка заголовков позволяет выявить основные параметры безопасности, такие как наличие X-Frame-Options для защиты от Clickjacking. В данном случае коллеги не настроили X-Frame-Options, так что можно проводить атаки, но мы конечно же, не будем.

### Проверка с помощью Nikto
...
`Nikto` выявляет стандартные уязвимости веб-серверов, такие как отсутствие определённых защитных заголовков, уязвимости SSL/TLS или старые версии ПО. В отчёте нам вывели, что не установлен X-Frame-Options и обнаружен wildcard-сертификат (это от ngrok'a, так что не релевантно), что мы уже знаем из предыдущей проверки.
...
```
`Nmap` позволяет анализировать поддерживаемые шифры и протоколы SSL/TLS. В результате можно выявить слабые алгоритмы или устаревшие версии протоколов. На данном сервере TLS 1.0 и 1.1 поддерживаются, что является устаревшей практикой (понимаем наших коллег и ни в чем не осуждаем) и может привести к компрометации соединения:(


### Проверка с помощью OpenSSL
А ещё `openssl` напоследок отдельно проверим:
```bash
...
root@irina-honor:~#
```
Команда используется для проверки деталей SSL-сертификата и параметров соединения. Из интересного можно увидеть цепочку сертификатов, используемые шифры и параметры ключей. В отчёте указано, что сертификат валиден, но сервер поддерживает только базовый набор алгоритмов, без расширенных настроек безопасности.

### Перебор директорий с помощью ffuf
Ну и теперь `ffuf`:
```bash
...
Инструмент используется для проверки на наличие скрытых директорий и файлов, доступ к которым мог быть не предусмотрен. НО так как мы с коллегами прописали правильный путь (`project/`), ffuf не выявил доступных директорий

---

Мы бы хотели и дальше повзламывать nginx, но все реквесты и лимиты на месяц в ngrok уже закончились, поэтому на этом всё!
